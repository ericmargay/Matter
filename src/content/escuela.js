  ...
  },
  {
    ...
    variantes: [
      ...
      V('g-mientras', 'Qué sí podemos hacer hoy', [],
        'Todo lo que no toca la tubería: el detector, la alarma sonora y el aviso al teléfono aunque no haya nadie en casa. Es la mayor parte del valor y no requiere acreditación. La válvula de corte se deja propuesta y se contrata aparte.'),
      V('g-nada-electrico', 'Qué NO debe hacer la automatización', [],
        'Con fuga de gas la automatización AVISA y nada más. No prende el extractor, no apaga luces, no mueve un relevador: un contacto eléctrico que abre o cierra hace chispa, y la chispa es lo que enciende el gas acumulado. Es al revés de lo que parece lógico —"que ventile solo"— y por eso hay que decirlo en voz alta. Se ventila abriendo una ventana con la mano, y el apagador no se toca.'),
      V('g-donde', 'Dónde va el detector', [],
        'A menos de metro y medio de la estufa y a unos 30 cm del piso. Cerca, porque es donde alguien deja una perilla abierta; bajo, porque el LP pesa más que el aire. Y NO encima de los quemadores: el vapor y el calor de cocinar disparan falsas alarmas y le acortan la vida al sensor. Necesita contacto cerca — se enchufa, no es de pila.'),
      V('g-bloqueo', 'El seguro de después de la alarma', [],
        'Detectada una fuga, la casa se queda quieta: ninguna automatización vuelve a mover un relevador hasta que una persona reconozca la alarma. Suena obvio dicho así, pero casi nadie lo programa — se piensa en el aviso y no en lo que sigue. Sin el seguro, el sensor de presencia del pasillo prende una luz cuando alguien va corriendo a la cocina, y esa chispa es igual de peligrosa que la del extractor. En la práctica se arma con un interruptor virtual de "modo seguro" que todas las automatizaciones consultan como condición, y que solo se apaga a mano o desde la app.'),
      V('g-sirena', 'Dónde va la sirena', [],
        'FUERA del cuarto donde puede haber gas. Encenderla cierra un relevador y eso hace chispa: ponerla en la cocina es hacer la chispa justo donde se acumula el gas. Va en el pasillo o en la recámara — que además es donde tiene que oírse, porque la sirena de la cocina no despierta a nadie que duerma con la puerta cerrada. Vale igual para la sirena Zigbee de marca y para la armada con enchufe inteligente.'),
